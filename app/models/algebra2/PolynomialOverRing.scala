...
class PolynomialOverRing private(val ring: Ring) extends Ring {
  ...
  val zeroPolynomial: Polynomial = builder(Map(0 -> ring.zero))
  val zero: Polynomial = zeroPolynomial

  val x: Polynomial = builder(Map(1 -> ring.one))

  val one: Polynomial = builder(Map(0 -> ring.one))
  ...
  object Polynomial {
    ...
    def apply(map: T1): T2 = {
      ...
      val normalMap = {
        ...
        def newMapList(oldMapList: List[(Int, ring.T2)]): List[(Int, ring.T2)] = oldMapList match {
          case Nil => Nil
          case ((x1, ring.zero) :: xs) => newMapList(xs) // TODO Revisar esto
    ...
    def comp(monomial1: (Int, ring.T2), monomial2: (Int, ring.T2)): Boolean = monomial1._1 > monomial2._1

  }

  class Polynomial private(val map: T1) extends RingElement{

    val fatherPolynomialOverRing: PolynomialOverRing = PolynomialOverRing.this
    val elementId: String = toString
    val isZero: Boolean = {
      val map = this.map
    ...
    def negate: Polynomial = this.multiply(ring.one.negate)
    ...
    def multiply(other: T2): Polynomial = { // TODO
    ...
    def *(other: ring.T2): Polynomial = this.multiply(other)
